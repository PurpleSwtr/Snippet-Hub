...
<template>
  <div class="space-y-2">
    ...
    <div class="border border-gray-200 dark:border-gray-700 rounded-md p-2 h-128 overflow-y-auto bg-gray-50 dark:bg-gray-900">
      ...
      <div v-else class="grid grid-cols-12 sm:grid-cols-12 gap-2">
        <button
          v-for="icon in filteredIcons"
          :key="icon"
          type="button"
          @click="selectIcon(icon)"
          class="aspect-square flex items-center justify-center p-1 rounded hover:cursor-pointer hover:bg-white dark:hover:bg-gray-800 border border-transparent hover:border-indigo-500 transition-all"
          :class="{'bg-white dark:bg-gray-800 border-indigo-500 ring-1 ring-indigo-500': modelValue === icon}"
          :title="icon"
        >
          <img :src="`/icons/all/${icon}`" class="w-16 h-16 object-contain" loading="lazy" />
        </button>
      </div>
    ...
    <div v-if="modelValue" class="text-xs text-gray-500">
      Выбрано: <span class="font-mono text-indigo-500">{{ modelValue }}</span>
    </div>
  </div>
</template>
